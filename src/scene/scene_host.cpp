...
void SceneHost::scene_loop()
{
    ...
    while (m_active) {
        ...
        if (m_requested_scene) {
            auto it = m_scenes.find(m_requested_scene);
            if (it != m_scenes.end() && it->second <= timeline_value) {
        ...
        if (scene) {
            ...
            while (m_event_queue.try_pop(evt))
                scene->handle_base_event(evt, this);
            scene->tick(frame_number, frame_time[1], frame_time[1] - frame_time[0], this);
