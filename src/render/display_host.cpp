...
#define DEMAND_FEATURE(STRUCTURE, FIELD)                                                                                              \
    if (STRUCTURE.FIELD == VK_FALSE) {                                                                                                \
        ...
        DEMAND_FEATURE(available_features.features, multiDrawIndirect);
        DEMAND_FEATURE(available_features11, storageBuffer16BitAccess);
        DEMAND_FEATURE(available_features12, bufferDeviceAddress);
        DEMAND_FEATURE(available_features12, descriptorBindingPartiallyBound);
        ...
        DEMAND_FEATURE(available_features12, runtimeDescriptorArray);
...
#undef DEMAND_FEATURE
        ...
        if (vkGetPhysicalDeviceImageFormatProperties(hwd, VK_FORMAT_D32_SFLOAT, VK_IMAGE_TYPE_2D, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT, 0, &ifmt) == VK_ERROR_FORMAT_NOT_SUPPORTED) {
...
int32_t DisplayHost::acquire_image()
{
    ...
    VkFence fence = m_fence_frame[next_frame_number % FRAMES_IN_FLIGHT];
    ...
    VkResult res;
    uint32_t index;
    VkSemaphore sem = m_sem_acquire_image[next_frame_number % FRAMES_IN_FLIGHT];
    do {
...
#else
        SDL_LogCritical(SDL_LOG_CATEGORY_GPU, "failed to acquire swapchain image: %d", res);
#endif
        return -1;
    }
}
...
void DisplayHost::present_image(uint32_t index, VkImage image, VkSemaphore signal)
{
    ...
    VkCommandBuffer present_commands = m_present_commands[frame_number % FRAMES_IN_FLIGHT];
    ...
    vkCmdPipelineBarrier(present_commands, VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT, 0, 0, nullptr, 0, nullptr, 1, &barrier);
    ...
    vkCmdBlitImage(present_commands, image, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, m_swapchain_images[index], VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &blit, VK_FILTER_NEAREST);

    barrier.srcAccessMask = VK_ACCESS_TRANSFER_WRITE_BIT;
    barrier.dstAccessMask = 0;
    barrier.oldLayout = VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL;
    barrier.newLayout = VK_IMAGE_LAYOUT_PRESENT_SRC_KHR;
    vkCmdPipelineBarrier(present_commands, VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_BOTTOM_OF_PIPE_BIT, 0, 0, nullptr, 0, nullptr, 1, &barrier);
    VK_DEMAND(vkEndCommandBuffer(present_commands));

    auto wait_sems = std::to_array<VkSemaphore>({ m_sem_acquire_image[frame_number % FRAMES_IN_FLIGHT], signal });
    auto wait_stages = std::to_array<VkPipelineStageFlags>({ VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT, VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT });
    VkSubmitInfo submit {};
    ...
    VK_DEMAND(vkQueueSubmit(queue, 1, &submit, m_fence_frame[frame_number % FRAMES_IN_FLIGHT]));
...
int DisplayHost::draw_frame(IRenderer* renderer)
{
